...
def get_tsne_results(list):
    ...
    source_embeddings = load_embeddings('ffhq', 'inception')
    pca = PCA(n_components=64, whiten=True)
    
    for idx, l in enumerate(list):
        target_embeddings = load_embeddings(l, 'inception')
...
for g in np.unique(labels):
    ...
    # ax.scatter(centroid[0], centroid[1], label = label, alpha = 1.0, s=5000, facecolor=color, marker="o",
    #             edgecolor='black', linewidth=1)
...
ax.set_title("Inception-v3 Features", fontsize=40, fontweight="bold")
fig.tight_layout()


ax.set_title("Inception-v3 Features", fontsize=40, fontweight="bold")
fig.savefig("{}/{}.pdf".format(save_dir, 'inception_fts_paper'), format="pdf", dpi=200, bbox_inches='tight')
